...
<div class="min-h-screen py-10 px-4 sm:px-6 lg:px-8 flex justify-center bg-gray-50 dark:bg-slate-950">
    ...
    <div class="w-full max-w-5xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-gray-200 dark:border-slate-800 flex flex-col md:flex-row">
        
        <div class="w-full md:w-1/3 bg-gray-100 dark:bg-slate-950/50 p-8 flex flex-col items-center border-b md:border-b-0 md:border-r border-gray-200 dark:border-slate-800">
            <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-6">{{ __('profile/edit.your_avatar') }}</h2>

            <div class="relative group mb-8">
                <div class="w-48 h-48 rounded-full border-4 border-white dark:border-slate-700 shadow-xl overflow-hidden relative bg-white">
                    <img id="avatar-preview" src="{{ Auth::user()->avatar_url }}" class="w-full h-full object-cover transition-transform duration-500 hover:scale-110">
                </div>
                <div class="absolute bottom-2 right-4 bg-orange-600 text-white p-2 rounded-full shadow-lg border-2 border-white dark:border-slate-900">
                    <i class="fas fa-camera"></i>
                </div>
            </div>

            <p class="text-sm text-gray-500 dark:text-slate-400 text-center mb-4 px-4">
        ...
        <div class="w-full md:w-2/3 p-8 md:p-12 bg-white dark:bg-slate-900">
            ...
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-8">
                    <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-3">{{ __('profile/edit.choose_character') }}</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', Auth::user()->name) }}" required
                               class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition font-medium">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{{ __('profile/edit.phone') }}</label>
                        <input type="text" name="telefono" value="{{ old('telefono', Auth::user()->telefono) }}" placeholder="{{ __('profile/edit.phone_placeholder') }}"
                               class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition font-medium">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="email" value="{{ Auth::user()->email }}" disabled
                                   class="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400 cursor-not-allowed font-mono text-sm">
                        </div>
                    </div>
                </div>

                <div class="bg-orange-50 dark:bg-orange-900/10 rounded-2xl p-6 border border-orange-100 dark:border-orange-900/20 mb-8">
                    <h3 class="text-sm font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wide mb-4 flex items-center gap-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="password" name="password" placeholder="••••••••" 
                                   class="w-full px-4 py-3 rounded-xl border border-orange-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500 outline-none transition">
                        </div>
                        <div>
                ...
                <div class="flex items-center justify-end gap-4">
                    <a href="{{ route('home') }}" class="px-6 py-3 text-sm font-bold text-slate-500 hover:text-slate-800 dark:hover:text-white transition">{{ __('profile/edit.btn_cancel') }}</a>
                    
                    <button type="submit" 
                            class="bg-orange-600 hover:bg-orange-500 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-orange-500/30 transform transition hover:-translate-y-0.5 active:scale-95 flex items-center gap-2">
...
<div id="modal-delete-account" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-md w-full p-8 border border-slate-200 dark:border-slate-700 text-center relative overflow-hidden transform transition-all">
