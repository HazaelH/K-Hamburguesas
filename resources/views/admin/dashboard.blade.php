...
<div class="space-y-8 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-end lg:items-center gap-6 bg-slate-800 p-6 rounded-3xl border border-slate-700 shadow-lg relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <p class="text-slate-400 mt-1">{{ __('admin/dashboard.subtitle') }}</p>
        </div>
        
        <div class="flex flex-wrap items-center gap-3 relative z-10 w-full lg:w-auto">
            <div class="hidden sm:flex bg-slate-900 border border-slate-700 p-1 rounded-xl">
                @php
                    $currentFilter = request('filter', 'month'); // Por defecto 'month'
                @endphp
                
                <a href="{{ route('admin.dashboard', ['filter' => 'today']) }}" 
                   class="px-4 py-1.5 text-xs font-bold transition rounded-lg {{ $currentFilter == 'today' ? 'text-white bg-orange-600 shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                   {{ __('admin/dashboard.filter_today') }}
                </a>
                
                <a href="{{ route('admin.dashboard', ['filter' => 'month']) }}" 
                   class="px-4 py-1.5 text-xs font-bold transition rounded-lg {{ $currentFilter == 'month' ? 'text-white bg-orange-600 shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                   {{ __('admin/dashboard.filter_month') }}
                </a>
                
                <a href="{{ route('admin.dashboard', ['filter' => 'year']) }}" 
                   class="px-4 py-1.5 text-xs font-bold transition rounded-lg {{ $currentFilter == 'year' ? 'text-white bg-orange-600 shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
            ...
            <a href="{{ route('admin.reports.daily') }}" target="_blank" class="bg-slate-900 hover:bg-slate-700 text-white border border-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition flex-1 justify-center shadow-inner group">
                <i class="fas fa-file-pdf text-red-500 group-hover:scale-110 transition-transform"></i> {{ __('admin/dashboard.btn_report') }}
            </a>
            
            <a href="{{ route('admin.reports.excel') }}" target="_blank" class="bg-slate-900 hover:bg-slate-700 text-white border border-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition flex-1 justify-center shadow-inner group">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-slate-800 border border-slate-700 p-6 rounded-3xl shadow-lg relative overflow-hidden group hover:border-emerald-500/50 transition-colors">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('admin/dashboard.revenue_month') }}</p>
                    <h3 class="text-3xl font-black text-white">${{ number_format($ingresosMensuales, 2) }}</h3>
                </div>
            ...
            <div class="flex items-center gap-2 text-xs">
                ...
                <span class="text-slate-500">{{ __('admin/dashboard.vs_last_month') }}</span>
            </div>
        </div>

        <div class="bg-slate-800 border border-slate-700 p-6 rounded-3xl shadow-lg relative overflow-hidden group hover:border-orange-500/50 transition-colors">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('admin/dashboard.pending_orders') }}</p>
                    <h3 class="text-3xl font-black text-white">{{ $pedidosPendientes }}</h3>
                </div>
        ...
        <div class="bg-slate-800 border border-slate-700 p-6 rounded-3xl shadow-lg relative overflow-hidden group hover:border-blue-500/50 transition-colors">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('admin/dashboard.active_menu') }}</p>
                    <h3 class="text-3xl font-black text-white">{{ $totalProductos }}</h3>
                </div>
        ...
        <div class="bg-slate-800 border border-slate-700 p-6 rounded-3xl shadow-lg relative overflow-hidden group hover:border-purple-500/50 transition-colors">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('admin/dashboard.registered_clients') }}</p>
            ...
            <div class="flex items-center gap-2 text-xs">
                ...
                <span class="text-slate-500">{{ __('admin/dashboard.this_week') }}</span>
            </div>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <a href="{{ route('kitchen.live') }}" class="bg-slate-800 border border-slate-700 hover:border-orange-500 p-5 rounded-3xl shadow-lg flex items-center justify-between group transition-all transform hover:-translate-y-1">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h4 class="text-white font-bold text-lg">{{ __('admin/dashboard.kitchen_monitor') }}</h4>
        ...
        <a href="{{ route('employee.pos') }}" class="bg-slate-800 border border-slate-700 hover:border-blue-500 p-5 rounded-3xl shadow-lg flex items-center justify-between group transition-all transform hover:-translate-y-1">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h4 class="text-white font-bold text-lg">{{ __('admin/dashboard.pos') }}</h4>
                    <p class="text-xs text-slate-400">{{ __('admin/dashboard.pos_desc') }}</p>
                </div>
            </div>
            <i class="fas fa-chevron-right text-slate-600 group-hover:text-blue-500 transition-colors"></i>
        </a>

        <a href="{{ route('admin.users.index') }}" class="bg-slate-800 border border-slate-700 hover:border-purple-500 p-5 rounded-3xl shadow-lg flex items-center justify-between group transition-all transform hover:-translate-y-1">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h4 class="text-white font-bold text-lg">{{ __('admin/dashboard.staff') }}</h4>
                    <p class="text-xs text-slate-400">{{ __('admin/dashboard.staff_desc') }}</p>
                </div>
            </div>
            <i class="fas fa-chevron-right text-slate-600 group-hover:text-purple-500 transition-colors"></i>
        </a>
    </div>

    {{-- GRÁFICAS (Reorganizadas) --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {{-- Gráfica de Ventas Históricas --}}
        <div class="lg:col-span-2 bg-slate-800 border border-slate-700 rounded-3xl shadow-xl p-6">
            <div class="flex justify-between items-center mb-6">
                <h6 class="font-black text-white text-lg flex items-center gap-2">
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-3xl shadow-xl p-6 flex flex-col justify-between">
            <h6 class="font-black text-white text-lg mb-2 flex items-center gap-2">
        ...
        {{-- NUEVA: Gráfica de Horas Pico (Ocupa todo el ancho en la segunda fila) --}}
        <div class="lg:col-span-3 bg-slate-800 border border-slate-700 rounded-3xl shadow-xl p-6">
            <div class="flex justify-between items-center mb-6">
                <h6 class="font-black text-white text-lg flex items-center gap-2">
    ...
    {{-- Y NO OLVIDES ACTUALIZAR EL CONTENEDOR DE DATOS OCULTO --}}
                        ...
                        <tr class="hover:bg-slate-700/30 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-bold text-slate-300">{{ $order->cliente_nombre ?? __('admin/dashboard.guest') }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-xs text-slate-500">{{ $order->created_at->diffForHumans(null, true, true) }}</td>
                            <td class="px-6 py-4 text-right">
                                <button onclick="window.openOrderModal({{ $order->id }})" 
                                        class="bg-slate-700/50 text-slate-300 hover:bg-orange-600 hover:text-white h-9 w-9 rounded-lg flex items-center justify-center ml-auto transition-all border border-slate-600 hover:border-orange-500" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                <div class="bg-slate-900 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-800">
                                    <i class="fas fa-receipt text-2xl text-slate-600"></i>
                                </div>
...
<div id="order-modal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4 sm:p-6 opacity-0 transition-opacity duration-300" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-md bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl overflow-hidden transform scale-95 transition-all duration-300 flex flex-col max-h-[90vh]" id="order-modal-panel">
        
        <div class="bg-slate-800 p-6 text-center border-b border-slate-700 relative overflow-hidden">
            <div class="absolute -right-4 -top-4 text-slate-700/30">
                <i class="fas fa-hamburger text-6xl"></i>
            </div>
            <h3 class="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">{{ __('admin/dashboard.modal_ticket') }}</h3>
            <p class="text-3xl font-black text-white font-mono">#<span id="modal-order-id">...</span></p>
            <span class="inline-block mt-3 bg-slate-900 text-slate-300 border border-slate-700 px-3 py-1 rounded-full text-xs font-bold" id="modal-status"></span>
            
            <button onclick="closeOrderModal()" class="absolute top-4 right-4 w-8 h-8 bg-slate-700/50 hover:bg-red-500 rounded-full flex items-center justify-center text-white transition-colors z-10">
        ...
        <div class="px-6 py-4 flex justify-between items-center text-sm border-b border-slate-800 bg-slate-900/50">
            <div>
                <p class="text-slate-500 text-xs font-bold uppercase tracking-wider">{{ __('admin/dashboard.modal_client') }}</p>
                <p class="text-white font-bold" id="modal-cliente">...</p>
            </div>
            <div class="text-right">
                <p class="text-slate-500 text-xs font-bold uppercase tracking-wider">{{ __('admin/dashboard.modal_time') }}</p>
                <p class="text-slate-300 font-mono" id="modal-fecha">...</p>
            </div>
        </div>

        <div class="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-900">
            <p class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-3">{{ __('admin/dashboard.modal_details') }}</p>
            <div id="modal-items-container" class="space-y-3">
            </div>
        </div>

        <div class="bg-slate-800 p-6 border-t border-slate-700 shadow-[0_-10px_20px_rgba(0,0,0,0.2)]">
            <div class="flex justify-between items-end mb-4">
                <span class="text-slate-400 font-bold uppercase tracking-wider">{{ __('admin/dashboard.modal_total') }}</span>
                <p class="text-3xl font-black text-emerald-400 font-mono">$<span id="modal-total">0.00</span></p>
            </div>
            <a href="#" id="modal-link-completo" class="w-full block text-center bg-orange-600 hover:bg-orange-500 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-orange-900/30">
...
<script>
    ...
    window.ADMIN_LANG = {
        ...
        orders: `{{ __('admin/dashboard.js_orders') }}` // <-- NUEVA VARIABLE
    };
</script>
