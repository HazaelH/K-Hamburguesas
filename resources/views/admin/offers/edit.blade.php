...
<div class="relative space-y-6 py-4 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.offers.index') }}" class="bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-xl transition-colors border border-slate-700 font-bold text-sm flex items-center gap-2">
        ...
        <form action="{{ route('admin.offers.update', $offer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" name="titulo" value="{{ old('titulo', $offer->titulo) }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 transition-all outline-none" required>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">🇲🇽 {{ __('admin/offers/offers.short_desc') }}</label>
                    <textarea name="descripcion" rows="2" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 transition-all outline-none">{{ old('descripcion', $offer->descripcion) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-900/50 p-5 rounded-xl border border-slate-700/50 space-y-4">
                <div>
                    ...
                    <select id="tipo_aplicacion" name="tipo_aplicacion" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 transition-all outline-none cursor-pointer">
                ...
                <div id="div_categoria" class="hidden transition-all duration-300">
                    <label class="block text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">{{ __('admin/offers/offers.select_category') }}</label>
                ...
                <div id="div_producto" class="hidden transition-all duration-300">
                    <label class="block text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">{{ __('admin/offers/offers.select_product') }}</label>
                    
                    <input type="hidden" id="referencia_producto" name="referencia_producto" value="{{ $offer->tipo_aplicacion == 'producto' ? $offer->referencia : '' }}">
                    
                    <button type="button" onclick="window.abrirModalProductos()" class="w-full bg-slate-900 border border-blue-500/50 hover:bg-slate-800 rounded-xl p-3 text-white focus:border-blue-500 outline-none flex justify-between items-center transition-colors">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="porcentaje" value="{{ old('porcentaje', $offer->porcentaje) }}" min="0" max="100" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 pl-10 text-white focus:border-blue-500 outline-none">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.50" name="precio_promo" value="{{ old('precio_promo', $offer->precio_promo) }}" placeholder="{{ __('admin/offers/offers.optional') }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 pl-10 text-white focus:border-blue-500 outline-none">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="fecha_inicio" value="{{ old('fecha_inicio', \Carbon\Carbon::parse($offer->fecha_inicio)->format('Y-m-d')) }}" class="flatpickr-date w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 outline-none" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{{ __('admin/offers/offers.end_date') }}</label>
                    <input type="text" name="fecha_fin" value="{{ old('fecha_fin', \Carbon\Carbon::parse($offer->fecha_fin)->format('Y-m-d')) }}" class="flatpickr-date w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 outline-none" required>
                </div>
            </div>

            <div class="p-4 bg-slate-900/50 rounded-xl border border-slate-700/50 flex items-center gap-6">
                @if($offer->imagen_url)
                    <img src="{{ asset('storage/' . $offer->imagen_url) }}" class="w-24 h-24 object-cover rounded-lg border border-slate-600 shadow-md">
                @else
                    <div class="w-24 h-24 bg-slate-800 rounded-lg border border-slate-700 flex items-center justify-center text-slate-600">
                        <i class="fas fa-image text-2xl"></i>
                    </div>
                @endif
                <div class="flex-1">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{{ __('admin/offers/offers.change_banner') }}</label>
                    <input type="file" name="imagen" class="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-slate-700 file:text-white hover:file:bg-slate-600 transition-colors cursor-pointer border border-slate-700 rounded-xl bg-slate-900">
            ...
            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)] hover:shadow-[0_0_25px_rgba(59,130,246,0.5)] transform hover:-translate-y-0.5">
                <i class="fas fa-save mr-2"></i> {{ __('admin/offers/offers.btn_save') }}
            </button>
        </form>
    </div>
</div>
    ...
    <div id="modal-productos-panel" class="bg-slate-900 border border-slate-700 p-6 rounded-3xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col transform scale-95 transition-all">
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
            <div class="relative flex-1">
                ...
                <input type="text" id="buscador_modal" placeholder="{{ __('admin/offers/offers.search_dish') }}" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2.5 pl-11 pr-4 text-white focus:border-blue-500 outline-none text-sm placeholder-slate-500">
            </div>
            ...
            <select id="filtro_categoria_modal" class="w-full sm:w-48 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:border-blue-500 outline-none text-sm cursor-pointer">
        ...
        <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="grid_productos_modal">
                @foreach($productos as $prod)
                    <div class="producto-item bg-slate-800 border border-slate-700 rounded-xl p-3 flex items-center gap-4 cursor-pointer hover:border-blue-500 hover:bg-slate-750 transition-colors group"
                        ...
                        @if($prod->imagen_url)
                            <img src="{{ asset('imagenes/' . $prod->imagen_url) }}" class="w-14 h-14 rounded-lg object-cover border border-slate-600 group-hover:border-blue-500/50">
                        @else
