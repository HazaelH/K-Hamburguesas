...
<div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 min-h-screen bg-gray-900">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-2/3 space-y-5">
                @foreach($carrito as $rowId => $item)
                    <div id="row-{{ $rowId }}" class="bg-gray-800/80 backdrop-blur-sm rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between border border-gray-300 hover:border-gray-600 shadow-lg relative group gap-5 transition-all duration-300"
                         data-precio-base="{{ $item['precio'] }}"> {{-- Data attribute crucial para el JS --}}
                        
                        <div class="flex items-center space-x-5 w-full sm:w-auto">
                            <div class="w-24 h-24 rounded-xl overflow-hidden flex-shrink-0 bg-gray-900 border border-gray-300 p-1">
                                <img src="{{ asset('imagenes/' . $item['imagen_url']) }}" alt="{{ $item['nombre'] }}" class="w-full h-full object-cover rounded-lg transform group-hover:scale-110 transition duration-500">
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <h2 class="text-lg font-bold text-white leading-tight mb-1 group-hover:text-orange-400 transition-colors">{{ $item['nombre'] }}</h2>
                                
                                @if(!empty($item['descripcion_mods']))
                                    <div class="inline-flex items-center gap-1.5 mt-1 bg-gray-900/60 px-2.5 py-1 rounded-md border border-gray-300/50">
                                ...
                                <p class="text-gray-400 font-medium mt-2 text-sm">
                                    {{ formatCurrency($item['precio']) }} <span class="text-xs text-gray-300">{{ __('client/cart.each') }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between w-full sm:w-auto gap-8">
                            ...
                            <div class="flex items-center bg-gray-900 rounded-xl border border-gray-300 p-1.5 shadow-inner">
                                <button type="button" aria-label="Disminuir cantidad" onclick="changeQty('{{ $rowId }}', 'decrease', this)" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition disabled:opacity-50 active:scale-95">
                                    <i class="fas fa-minus text-xs"></i>
                                </button>

                                <span id="qty-{{ $rowId }}" class="w-10 text-center font-black text-white text-base">{{ $item['cantidad'] }}</span>

                                <button type="button" aria-label="Aumentar cantidad" onclick="changeQty('{{ $rowId }}', 'increase', this)" class="w-8 h-8 flex items-center justify-center text-orange-500 hover:text-white hover:bg-orange-700 rounded-lg transition disabled:opacity-50 active:scale-95">
                                    <i class="fas fa-plus text-xs"></i>
                                </button>
                            </div>

                            <div class="text-right min-w-[85px]">
                                <span class="block text-[10px] text-gray-300 uppercase font-bold tracking-wider mb-0.5">{{ __('client/cart.total') }}</span>
                                <span class="text-xl font-black text-white" id="item-total-{{ $rowId }}">
                                    {{ formatCurrency($item['precio'] * $item['cantidad']) }}
                                </span>
                            </div>

                            <button onclick="removeCartItem(this, '{{ $rowId }}')" class="w-10 h-10 rounded-full flex items-center justify-center text-gray-300 bg-gray-900/50 hover:bg-red-500/10 hover:text-red-500 transition-colors border border-transparent hover:border-red-500/30 group" title="{{ __('client/cart.delete') }}">
            ...
            <div class="lg:w-1/3">
                <div class="bg-gradient-to-b from-gray-800 to-gray-900 rounded-3xl p-6 sm:p-8 border border-gray-300 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.7)] sticky top-28">
                    
                    <h2 class="text-xl font-black text-white mb-6 flex items-center">
                        <i class="fas fa-receipt text-gray-300 mr-3"></i> {{ __('client/cart.purchase_summary') }}
                    </h2>
                    ...
                    <div class="mt-4 flex items-center justify-center text-xs text-gray-300 font-medium gap-2">
    ...
    @else
        <div class="flex flex-col items-center justify-center py-24 sm:py-32 bg-gray-800/30 rounded-3xl border-2 border-dashed border-gray-300/50 mt-10">
            ...
            <div class="relative w-32 h-32 flex items-center justify-center mb-6">
                ...
                <div class="relative bg-gray-800 border border-gray-300 w-24 h-24 rounded-full flex items-center justify-center shadow-2xl">
