...
<div class="flex flex-col lg:flex-row h-full w-full overflow-hidden bg-slate-950 relative selection:bg-orange-500/30">

    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        <div class="absolute top-[-10%] left-[-5%] w-[400px] h-[400px] bg-orange-600/20 rounded-full blur-[120px]"></div>
        <div class="absolute bottom-[-10%] right-[20%] w-[500px] h-[500px] bg-blue-600/10 rounded-full blur-[120px]"></div>
    </div>

    <div class="flex-1 min-w-0 flex flex-col h-[55%] lg:h-full z-10 relative border-b lg:border-b-0 lg:border-r border-slate-800 bg-slate-900/40 backdrop-blur-xl order-1 overflow-hidden">
            ...
            <div class="flex justify-between items-center gap-4">
                ...
                <div class="relative group flex-1 max-w-md">
                    ...
                    <input type="text" id="buscador-pos" placeholder="{{ __('employee/pos/index.search_placeholder') }}" 
                           class="w-full bg-slate-800/80 text-white border border-slate-700 rounded-2xl py-3 pl-12 pr-4 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 shadow-inner transition-all text-base lg:text-lg placeholder-slate-500">
                </div>
            </div>
            ...
            <div class="w-full">
                <div class="flex flex-wrap gap-2 pt-1 pb-2" id="category-filters" role="tablist">
                    ...
                    <button onclick="filterCategory('all', this)" role="tab" aria-selected="true"
                            class="cat-btn active px-5 py-2.5 bg-orange-600 text-white rounded-xl font-bold text-sm shadow-md transition-transform active:scale-95 focus:outline-none focus:ring-2 focus:ring-orange-400 flex items-center gap-2">
                    ...
                    @foreach($categoriasBd as $cat)
                        ...
                        <button onclick="filterCategory('{{ $cat->nombre }}', this)" role="tab" aria-selected="false"
                                class="cat-btn px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-700 rounded-xl font-bold text-sm transition-transform active:scale-95 focus:outline-none focus:ring-2 focus:ring-slate-400 flex items-center gap-2">
        ...
        <div class="flex-1 overflow-y-auto p-4 lg:p-6 pt-0 custom-scrollbar min-h-0">
            <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5" id="grid-productos">
                @foreach($productos as $producto)
                    {{-- Pasamos el nombre traducido al JS del POS --}}
                    <div onclick="openCustomizationModal({{ $producto->id_producto }}, '{{ addslashes($producto->nombre_traducido) }}', {{ $producto->precio }}, '{{ asset('imagenes/' . $producto->imagen_url) }}', {{ json_encode($producto->opciones_personalizacion ?? '[]') }}, '{{ app()->getLocale() }}')"
                        class="product-item group relative bg-slate-800/80 backdrop-blur-sm border border-slate-700 rounded-2xl overflow-hidden cursor-pointer transition-all duration-200 hover:border-orange-500 hover:shadow-[0_0_20px_rgba(234,88,12,0.15)] active:scale-95 flex flex-col h-full min-h-[160px] select-none"
                        ...
                        <div class="h-28 lg:h-32 w-full relative overflow-hidden bg-slate-900 shrink-0">
                            @if($producto->imagen_url)
                                <img src="{{ asset('imagenes/' . $producto->imagen_url) }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            @else
                                <div class="w-full h-full flex items-center justify-center text-slate-700 text-3xl"><i class="fas fa-hamburger"></i></div>
                            @endif
                            
                            {{-- Aplicamos el Helper de Moneda --}}
                            <div class="absolute bottom-2 right-2 bg-slate-900/90 backdrop-blur-md rounded-lg px-2.5 py-1 text-white text-sm font-black border border-slate-700 shadow-lg">
    ...
    <div class="w-full lg:w-80 xl:w-[380px] shrink-0 flex flex-col h-[45%] lg:h-full min-h-0 bg-slate-900/95 shadow-2xl z-20 border-l border-slate-800 relative order-2">
        
        <div class="lg:hidden w-full flex justify-center pt-3 pb-2 bg-slate-900 cursor-pointer" onclick="toggleCartMobile()">
            <div class="w-16 h-1.5 bg-slate-700 rounded-full"></div>
        </div>

        <div class="p-4 lg:p-5 bg-slate-900 border-b border-slate-800 shrink-0 space-y-4">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-1 gap-3">
                <div class="relative">
                    <span class="absolute left-3 top-3 text-slate-400"><i class="fas fa-utensils"></i></span>
                    <select id="pos-mesa" class="w-full bg-slate-800 text-white rounded-xl py-3 pl-10 pr-10 text-sm font-bold border border-slate-700 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none appearance-none cursor-pointer">
                        <option value="">{{ __('employee/pos/index.takeout') }}</option>
                        @for($i=1; $i<=15; $i++) <option value="{{$i}}">{{ __('employee/pos/index.table', ['number' => $i]) }}</option> @endfor
                    </select>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="pos-cliente" placeholder="{{ __('employee/pos/index.client_name') }}" 
                           class="w-full bg-slate-800 text-white rounded-xl py-3 pl-10 pr-4 text-sm border border-slate-700 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none placeholder-slate-500">
                </div>
            </div>
        </div>
        ...
        <div class="p-4 lg:p-5 bg-slate-900 border-t border-slate-800 shrink-0 z-30">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <label class="cursor-pointer relative">
                    ...
                    <div class="bg-slate-800 border-2 border-slate-700 text-slate-400 py-2.5 rounded-xl text-center font-black uppercase transition-all flex flex-col items-center gap-1
            ...
            <button id="btn-pagar" onclick="submitOrder()" disabled
                    class="w-full bg-orange-600 hover:bg-orange-500 text-white font-black py-4 rounded-xl shadow-lg shadow-orange-900/30 transition-all flex justify-center items-center gap-3 text-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-orange-600 active:scale-[0.98]">
        ...
        <div id="customization-modal" class="fixed inset-0 z-[150] hidden flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 opacity-0 transition-opacity duration-300">
            <div id="customization-panel" class="bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] transform scale-95 transition-all duration-300">
                
                <div class="p-5 border-b border-slate-800 flex justify-between items-start bg-slate-800/50 rounded-t-3xl">
                    <div class="flex gap-4 items-center">
                        <img id="modal-prod-img" src="" class="w-16 h-16 rounded-xl object-cover border-2 border-slate-700 shadow-lg bg-slate-900">
                        <div>
                    ...
                    <button onclick="closeCustomizationModal()" class="w-8 h-8 bg-slate-800 hover:bg-red-500 text-slate-400 hover:text-white rounded-full flex items-center justify-center transition-colors">
                ...
                <div class="p-5 border-t border-slate-800 bg-slate-800/50 rounded-b-3xl shrink-0">
                    ...
                    <button id="modal-add-btn" class="w-full bg-orange-600 hover:bg-orange-500 text-white font-black py-4 rounded-xl shadow-lg transition-all active:scale-95 flex justify-center items-center gap-2">
...
{{-- Puente de datos hacia JavaScript --}}
<script>
    const API_STORE_ORDER = "{{ route('employee.pos.store') }}";
    const CSRF_TOKEN = "{{ csrf_token() }}";

    // Variables de Moneda para que POS.js sepa cómo formatear
    window.MENU_LANG = {
