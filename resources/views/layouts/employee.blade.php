...
<head>
    ...
    <title>@yield('titulo') - {{ __('layouts/employee.employee_panel') }}</title>
...
<body class="h-screen flex flex-col overflow-hidden bg-slate-950">
    ...
    <nav class="bg-slate-900 border-b border-slate-700 h-16 shrink-0 shadow-md relative z-50">
        <div class="w-full h-full px-4 md:px-6 flex justify-between items-center">
            ...
            <a href="{{ route('employee.panel') }}" class="flex items-center gap-3 hover:opacity-80 transition shrink-0">
                ...
                <div>
                    <span class="font-bold text-lg tracking-wide text-white block leading-none">{{ strtoupper(__('layouts/employee.k_hamburguesas')) }}</span>
                    <span class="text-[10px] text-orange-500 block font-mono uppercase tracking-[0.2em] mt-1">{{ strtoupper($rol) }}</span>
                </div>
            </a>
            ...
            <div class="hidden lg:flex items-center gap-2 bg-slate-800 p-1 rounded-xl border border-slate-700">
                ...
                <a href="{{ route('admin.dashboard') }}" class="px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap bg-slate-900 border border-slate-600 text-white hover:bg-slate-700 hover:border-slate-400 shadow-inner" title="{{ __('layouts/admin.admin_panel') }}">
                    <i class="fas fa-user-shield text-orange-500"></i> <span>{{ __('layouts/employee.admin_mode') }}</span>
                </a>
                <div class="w-px h-5 bg-slate-600 mx-1"></div>
                @endif
                
                @if(in_array($rol, ['admin', 'mesero', 'cajero']))
                <a href="{{ route('employee.pos') }}" class="px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap {{ request()->routeIs('employee.pos') ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-700' }}">
                    <i class="fas fa-cash-register"></i> <span>{{ __('layouts/employee.sale') }}</span>
                </a>
                @endif
                ...
                <a href="{{ route('employee.orders.index') }}" class="px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap {{ request()->routeIs('employee.orders.index') ? 'bg-violet-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-700' }}">
                    <i class="fas fa-wallet"></i> <span>{{ __('layouts/employee.cash_register') }}</span>
                </a>
                @endif
                ...
                <a href="{{ route('stock.index') }}" class="px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap {{ request()->routeIs('stock.index') ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-700' }}">
                    <i class="fas fa-boxes"></i> <span>{{ __('layouts/employee.stock') }}</span>
                </a>
                @endif

                @if(in_array($rol, ['admin', 'repartidor']))
                <a href="{{ route('delivery.scan') }}" class="px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap {{ request()->routeIs('delivery.scan') ? 'bg-pink-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-700' }}">
                    <i class="fas fa-motorcycle"></i> <span>{{ __('layouts/employee.delivery') }}</span>
                </a>
                @endif
            </div>
                ...
                <div class="relative border-r border-slate-700 pr-4 mr-1">
                    ...
                    <div id="lang-dropdown-emp" class="absolute right-0 mt-4 w-40 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl hidden z-50 overflow-hidden">
                        <div class="p-2 space-y-1">
                            ...
                            <button disabled class="flex items-center gap-3 px-3 py-2 rounded-lg opacity-50 cursor-not-allowed text-sm font-bold text-slate-500 w-full text-left">
        ...
        <div id="mobile-menu" class="absolute top-16 left-0 w-full bg-slate-900 border-b border-slate-700 shadow-2xl lg:hidden z-40">
            <div class="p-4 space-y-2">
                ...
                @if(in_array($rol, ['admin', 'mesero', 'cajero']))
                <a href="{{ route('employee.pos') }}" class="block px-4 py-3 rounded-lg font-bold items-center gap-3 {{ request()->routeIs('employee.pos') ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"><i class="fas fa-cash-register w-6 text-center"></i> {{ __('layouts/employee.sale') }}</a>
                @endif

                @if(in_array($rol, ['admin', 'cajero']))
                <a href="{{ route('employee.orders.index') }}" class="block px-4 py-3 rounded-lg font-bold items-center gap-3 {{ request()->routeIs('employee.orders.index') ? 'bg-violet-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"><i class="fas fa-wallet w-6 text-center"></i> {{ __('layouts/employee.cash_register') }}</a>
                @endif
                ...
                @if(in_array($rol, ['admin', 'repartidor']))
                <a href="{{ route('delivery.scan') }}" class="block px-4 py-3 rounded-lg font-bold items-center gap-3 {{ request()->routeIs('delivery.scan') ? 'bg-pink-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"><i class="fas fa-motorcycle w-6 text-center"></i> {{ __('layouts/employee.delivery') }}</a>
                @endif
                ...
                <div class="border-t border-slate-700 mt-4 pt-4">
                    ...
                    <a href="{{ route('profile.edit') }}" class="w-full text-left px-4 py-3 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white font-bold flex items-center gap-3 transition">
                        <i class="fas fa-user-circle w-6 text-center"></i> {{ __('layouts/employee.my_profile') }}
                    </a>
    ...
    <main class="flex-1 overflow-y-auto relative bg-slate-900 custom-scrollbar">
        @yield('contenido')
    </main>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(btn && menu) {
                ...
                document.addEventListener('click', (e) => {
                    ...
                    if(langDrop && !langBtn.contains(e.target) && !langDrop.contains(e.target)){
